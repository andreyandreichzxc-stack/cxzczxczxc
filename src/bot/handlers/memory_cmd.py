...
from src.db.repo import add_memory, delete_memory, get_or_create_user, list_memories, search_memories
...
async def cmd_remember(message: Message, command: CommandObject) -> None:
    ...
    if not args:
        await message.answer("Использование: <code>/remember [контакт] факт</code>\nПример: <code>/remember Настя злится</code>")
        return
    ...
    async with get_session() as session:
        owner = await get_or_create_user(session, message.from_user.id)
        mem = await add_memory(session, owner, fact=fact, contact_id=contact_id, source="user")
