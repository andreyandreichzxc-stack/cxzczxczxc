...
11.4) "store_memory" — запомнить факт о владельце или контакте.
    Параметры:
      "fact": "один факт (одно предложение)",
      "contact": null | "имя контакта, если факт о нём",
      "sentiment": null | "positive" | "negative" | "neutral".
    Используй для «запомни: я болею астмой», «Настя злится из-за сорванного дедлайна»,
    «запиши что мы помирились с Настей». Если новый факт противоречит старому —
    ТОЛЬКО этот новый сохраняй (старый сам собой устареет).
